  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      ...
      if (data) {
        ...
        setError("Unable to find product details.");
      }
      setLoading(false);
    };
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center py-32 px-6">
        ...
        <p className="text-sm text-muted-foreground mt-2 max-w-xs">
          {error || "The product you are looking for is currently unavailable."}
        </p>
        <Link
          href="/newProduct"
          className="mt-8 bg-primary text-primary-foreground font-semibold px-8 py-3 rounded-full hover:opacity-90 transition-opacity text-xs tracking-wider animate-fade-in"
        >
    ...
    <main className="flex-1 bg-background text-foreground transition-colors duration-300 py-12 px-6 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-start">
          ...
          <div className="lg:col-span-6 flex flex-col gap-4 items-center w-full animate-fade-in-delay-1">
            ...
            {product.images && product.images.length > 1 && (
              <div className="flex gap-3 mt-2 overflow-x-auto max-w-[500px] pb-2 scrollbar-thin">
                {product.images.map((img, idx) => (
                  <button
                    key={idx}
                    onClick={() => setActiveImage(img)}
                    className={`w-16 h-16 rounded-xl border p-1 glass flex items-center justify-center cursor-pointer transition-all duration-300 ${
                      activeImage === img ? "border-primary scale-105 shadow-md shadow-primary/10" : "border-border/40 hover:border-primary/40"
                    }`}
                  >
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={img} alt={`${product.title} view ${idx + 1}`} className="max-h-full max-w-full object-contain" />
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="lg:col-span-6 flex flex-col gap-8 w-full animate-fade-in-delay-2">
            <div>
              ...
              <div className="flex gap-2 items-center flex-wrap">
                ...
                {product.brand && (
                  <span className="text-[10px] font-bold tracking-[0.25em] text-muted-foreground bg-muted border border-border/40 px-4 py-1.5 rounded-full uppercase">
                    {product.brand}
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center gap-4 mt-4 flex-wrap">
                <span className="text-2xl sm:text-3xl font-extrabold text-foreground">${Math.round(product.price)}</span>
                <span className={`text-[10px] font-bold tracking-wider px-3 py-1 rounded-full uppercase border ${
                  product.stock > 10 ? "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" : "text-amber-500 bg-amber-500/10 border-amber-500/20"
                }`}>
            ...
            {/* Interactive Actions: Qty and Add to Cart */}
            ...
            <div className="grid grid-cols-3 gap-2 text-center py-2.5 border-t border-b border-border/20 mt-2 text-[10px] tracking-wider text-muted-foreground font-medium uppercase">
              <div className="flex flex-col gap-1 items-center justify-center">
                <span>🛡️ {product.warrantyInformation || "2-Year Warranty"}</span>
              </div>
